...
{
	VIP_RegisterFeature(g_sFeature[0], STRING, _, OnToggleItem);
...
{
	#if DEBUG_MODE	
	BuildPath(Path_SM, g_sDebugLogFile, sizeof(g_sDebugLogFile), "logs/g_sFeature[0]_Debug.log");
	#endif
...
Handle:CreateSkinsMenu(iClient)
{
	...
	if(KvGotoFirstSubKey(g_hKeyValues))
	{
		...
		VIP_GetClientFeatureString(iClient, g_sFeature[0], sClientSkins, sizeof(sClientSkins));
...
public Event_PlayerTeam(Handle:hEvent, const String:name[], bool:dontBroadcast)
{
	...
	if(iClient && VIP_IsClientVIP(iClient) && VIP_GetClientFeatureStatus(iClient, g_sFeature[0]) != NO_ACCESS)
	{
...
{
	if(g_bIsCSGO == false && bIsVIP && VIP_IsClientFeatureUse(iClient, g_sFeature[0])) SetClientVIPSkin(iClient);
}

public Event_PlayerSpawn(Handle:hEvent, const String:name[], bool:dontBroadcast)
{
	new iClient = GetClientOfUserId(GetEventInt(hEvent, "userid"));
	if(VIP_IsClientVIP(iClient) && VIP_IsClientFeatureUse(iClient, g_sFeature[0]) && IsPlayerAlive(iClient))
	{
		SetClientVIPSkin(iClient);
		SetClientVIPArms(iClient);
	}
